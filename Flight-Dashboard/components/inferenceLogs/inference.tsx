...
import { goFront, stopFront, goBack, stopBack, goLeft, stopLeft, goRight, stopRight, goStop, stopStop } from "../../a/flight-simulation/src/controls";
...
function generateDummyData() {
  ...
  for (let i = 0; i < 10; i++) {
    data.push({
      ...
      gpsPositioning: { lat: 35.6895 + Math.random() * 0.01, lon: 139.6917 + Math.random() * 0.01 },  // GPS coordinates
      altitude: 500 + Math.random() * 50,  // Altitude in meters
      attitude: { pitch: Math.random() * 10, roll: Math.random() * 10, yaw: Math.random() * 10 },  // Attitude in degrees
      speed: { groundSpeed: 50 + Math.random() * 5, airSpeed: 100 + Math.random() * 5 }  // Speed in km/h
    });
  }
  return data;
}
  ...
  console.log(type)
    if (type === 1) {
      ...
      return [
        ...
        return [
        { "decision": "ascend", "reason": "500m below safety zone" },
        { "decision": "ascend", "reason": "reach optimal altitude" },
        { "decision": "ascend", "reason": "5° off course" },
        { "decision": "ascend", "reason": "correct path alignment" },
        { "decision": "right", "reason": "adjust for wind" },
        { "decision": "hold", "reason": "maintain current settings" },
        { "decision": "descend", "reason": "prepare for landing" },
        { "decision": "right", "reason": "final landing approach" },
...
export function InferenceLogs({ type, handleGraphData }) {
    ...
    return (
        <div style={{ backgroundColor: '#000', color: '#0f0', fontFamily: 'monospace', padding: '10px', height: '400px', overflowY: 'auto' }}>
            {logs.map((log, index) => (
